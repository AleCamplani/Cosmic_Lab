...
int interruptPin = 2; //Pin we get interrupt signal from
...
void loop() {
  ...
  detachInterrupt(digitalPinToInterrupt(interruptPin)); //Do not interrupt when talking to GPS
  ...
  attachInterrupt(digitalPinToInterrupt(interruptPin),event,RISING);
...
void printTime(Adafruit_GPS GPS, unsigned long microTimer_offset, unsigned long now){//Takes GPS as input along with internal timer and prints time to serial connection
  ...
  unsigned long microsecond = microTimer%1000; //modulo since we only want last three digits for printing
  int millisecond = (microTimer/1000)%1000; //modulo since we only want last three digits for printing
  int second = (GPS.seconds + microTimer/1000000)%60; //more than 60 should be new minute
  int minute = (GPS.minute + (GPS.seconds + microTimer/1000000)/60)%60;
  int hour = (GPS.hour+(GPS.minute + (GPS.seconds + microTimer/1000000)/60)); //assume we do not need to worry about data takan at midnight
