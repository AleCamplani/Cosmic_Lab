...
Need timer0 to count between PPS
Need timer1 to timestamp events
...
void setup() {
  ...
  //set timer0 up for counting cycles
...
unsigned long NanosecondsNow(){
  return (overflowCounter * timer0TOP + TCNT0)*clkSpeed;
}

void PPS(){//interrupt from GPS 
  cyclesInSecond = overflowCounter * timer0TOP + TCNT0; //How many cycles passed since last PPS event
...
ISR(TIMER0_COMPA_vect){ //Run the ISR for Timer0
  overflowCounter=overflowCounter+1; //Increment counter
  TCNT1=TCNT0;//Keep other in synch
}
...
ISR(TIMER1_CAPT_vect){ //ISR for events on the pin
  if (EventFlag==false){ //only record if we have lowered flag from last event
    ...
    EventNanosec = (overflowCounter * timer0TOP + combined)*clkSpeed; //#cylces times clockspeed (since second ticked over)
    EventSec=second;
