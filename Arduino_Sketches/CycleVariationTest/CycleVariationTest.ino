/*
This Script will log the variation in the number of cycles we record in each second.
...
60ns precision. Timer TCB0 is used for listening to the PPS from the GPS. 
TCB2 keeps the time between PPS events. 
...
ISR(TCB0_INT_vect){ //PPSfunc
  ...
  EventFlag = true; //We want to log number in one esecond
  
  clkSpeed=1e9/cyclesInSecond; //Recompute clockspeed (in ns)
  CounterNow = TCB2.CNT;
  TCB2.CNT=TCB2.CNT-CounterNow; //reset counters so that they are zero at PPS
  TCB1.CNT=TCB2.CNT;
  TCB0.CNT=TCB2.CNT;
