...
plt.close('all')
filename='..\..\Datafiles\Coicidence3Run2.txt' #File currently being written to
...
while((time.time()-Start)<timeout):
    ...
    plt.figure(1)
    plt.hist(time_deltas,bins=int(np.sqrt(len(time_deltas))),range=(0,np.max(time_deltas)),color="red")
    plt.pause(0.05)
